 ...
 * @subpackage Mappers
 */
abstract class Mapper extends OrmObject implements IMapper
{
	...
	public function getConventional()
	{
		if ($this->conventional === NULL)
		{
			...
			{
				throw new BadReturnException(array($this, 'createConventional', 'IConventional', $conventional));
			}
	...
	final protected function getCollectionClass($info = false)
	{
		if ($this->collectionClass === NULL)
		{
			...
			{
				throw new BadReturnException(array($this, 'createCollectionClass', 'IEntityCollection class name', NULL, "; '{$class}' doesn't exists"));
			}
			$reflection = new ReflectionClass($class);
			if (!$reflection->implementsInterface('IEntityCollection'))
			{
				throw new BadReturnException(array($this, 'createCollectionClass', 'IEntityCollection class name', NULL, "; '{$class}' must implement IEntityCollection"));
			}
			else if ($reflection->isAbstract())
			{
				throw new BadReturnException(array($this, 'createCollectionClass', 'IEntityCollection class name', NULL, "; '{$class}' is abstract."));
			}
			else if (!$reflection->isInstantiable())
			{
				throw new BadReturnException(array($this, 'createCollectionClass', 'IEntityCollection class name', NULL, "; '{$class}' isn't instantiable."));
			}
