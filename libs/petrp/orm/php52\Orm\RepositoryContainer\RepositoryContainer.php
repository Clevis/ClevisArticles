 ...
 */
class RepositoryContainer extends OrmObject implements IRepositoryContainer
{
	...
	public function __construct($containerFactory = NULL)
	{
		...
		{
			throw new OrmInvalidArgumentException(array($this, '__construct() first param', 'IServiceContainerFactory, IServiceContainer or NULL', $containerFactory));
		}
	}
	...
	public static function get()
	{
		...
		{
			throw new OrmDeprecatedException(__CLASS__ . '::get() is deprecated do not use it.');
		}
	...
	final private function checkRepositoryClass($class, $name, $throw = true, & $originClass = NULL)
	{
		...
		{
			if (!$throw) return false;
			throw new RepositoryNotFoundException("Repository '{$originClass}' must implement IRepository");
		}
	...
	final protected function getRepositoryClass($name)
	{
		...
		{
			throw new OrmDeprecatedException(array(__CLASS__, 'getRepositoryClass() is deprecated; repositoryName', 'class name'));
		}
		if (!$name) return NULL;
	...
	{
		throw new OrmDeprecatedException(array($this, 'setContext()', $this, '__construct()'));
	}
}
