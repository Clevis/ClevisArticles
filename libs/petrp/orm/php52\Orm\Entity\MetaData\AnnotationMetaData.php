 ...
 */
class AnnotationMetaData extends OrmObject
{
	 ...
	 * @return MetaData
	 */
	public static function getMetaData($metaData, OrmAnnotationsParser $parser = NULL)
	{
		...
		if ($parser === NULL)
		{
			$parser = new OrmAnnotationsParser();
		}
		new self($metaData, $parser);
		return $metaData;
	}

	/**
	 * @param MetaData
	 * @param OrmAnnotationsParser
	 */
	protected function __construct(MetaData $metaData, OrmAnnotationsParser $parser)
	{
		$this->parser = $parser;
	...
	{
		if ($mode === MetaData::READWRITE) // bc; drive OrmAnnotationsParser na pomlcce zkoncil
		{
	...
	public function builtParamsEnum($string)
	{
		...
		{
			$enum = OrmCallback::create($this->parseSelf($tmp[1]))->invoke();
