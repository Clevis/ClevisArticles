 ...
 */
class AnnotationClassParser extends OrmObject
{
	...
	public function __construct(OrmAnnotationsParser $parser)
	{
		$this->parser = $parser;
	}
	...
	/**
	 * <code>
	 * 	$p->register('mapper', 'IRepository', function ($repositoryClass) {
	...
	public function register($annotation, $interface, $defaultClassFallback = NULL)
	{
		...
		if ($defaultClassFallback !== NULL AND !is_callable($defaultClassFallback) AND !OrmCallback::is($defaultClassFallback))
		{
	...
	{
		throw new OrmDeprecatedException(array(__CLASS__, 'getAnnotations()', '', __CLASS__ . '->parser->getByReflection()'));
	}
