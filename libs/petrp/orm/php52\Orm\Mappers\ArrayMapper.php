...
abstract class ArrayMapper extends Mapper
{
		...
		try {
			...
			foreach ($this->data as $id => $entity)
			{
				if ($entity)
				{
					...
					if ($arguments['params'])
					{
						throw new OrmNotSupportedException('EventArguments::$params are not supported for ArrayMapper during Events::SERIALIZE_BEFORE event.');
					}
	...
	{
		throw new OrmNotImplementedException(get_class($this) . '::loadData() is not implement, you must override and implement that method');
	}
	...
	{
		throw new OrmNotImplementedException(get_class($this) . '::saveData() is not implement, you must override and implement that method');
	}
