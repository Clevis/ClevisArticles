 ...
 */
class PerformanceHelper extends OrmObject
{

	/**
	 * OrmCallback pro klic pod kterym se uklada, defaultne je to url stranky.
	 * @var callback
	 */
	...
	public function __construct(IRepository $repository, ArrayAccess $cache)
	{
		...
		if (self::$toLoad === NULL)
		{
			$key = self::$keyCallback ? (string) OrmCallback::create(self::$keyCallback)->invoke() : NULL;
			$key = $key ? $key : '*';
			if (strlen($key) > 50)
			{
	...
	{
		throw new OrmDeprecatedException(array(__CLASS__, 'getCache()', 'constructor injection'));
	}
