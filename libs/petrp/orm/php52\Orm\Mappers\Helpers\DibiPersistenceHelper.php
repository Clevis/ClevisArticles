 ...
 */
class DibiPersistenceHelper extends OrmObject
{
	...
	protected function toArray(IEntity $entity, $id, $operation)
	{
		...
		foreach ($params as $key => $do)
		{
			...
			if ($do !== true)
			{
				$value = OrmCallback::create($do)->invoke($value, $entity);
			}
	...
	{
		throw new OrmDeprecatedException(array(__CLASS__, '$connection setter', 'constructor'));
	}

	/** @deprecated */
	final public function setConventional()
	{
		throw new OrmDeprecatedException(array(__CLASS__, '$conventional setter', 'constructor'));
	}

	/** @deprecated */
	final public function getMapper()
	{
		throw new OrmDeprecatedException(array(__CLASS__, '$mapper'));
	}
