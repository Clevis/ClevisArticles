 ...
 */
abstract class Repository extends OrmObject implements IRepository
{
	...
	public function __construct(IRepositoryContainer $model)
	{
		...
		{
			throw new BadReturnException(array($this, 'createPerformanceHelper', 'PerformanceHelper or NULL', $ph));
		}
	...
	final public function getById($id)
	{
		...
		{
			throw new OrmInvalidArgumentException(array($this, 'getById() $id', 'scalar', $id));
		}
	...
	final public function persist(IEntity $entity, $all = true)
	{
		...
		if (!$recursionRelationshipCallback) $recursionRelationshipCallback = create_function('IEntity $entity, array & $recursionRelationship, $all, $persist = false', '
	...
	final public function flush()
	{
		...
		{
			throw new OrmDeprecatedException(array(__CLASS__, 'flush(TRUE)'));
		}
	...
	final public function clean()
	{
		if (func_num_args() > 0 AND func_get_arg(0))
		{
			throw new OrmDeprecatedException(array(__CLASS__, 'clean(TRUE)'));
		}
	...
	final public function getMapper()
	{
		if ($this->mapper === NULL)
		{
			...
			if (!($mapper instanceof IMapper))
			{
				if (is_object($mapper))
				{
					throw new BadReturnException('Mapper ' . get_class($mapper) . ' must implement IMapper');
				}
				throw new BadReturnException(array($this, 'createMapper', 'IMapper', $mapper));
			}
			$this->mapper = $mapper;
		}
		return $this->mapper;
	...
	public function getEntityClassName(array $data = NULL)
	{
		...
		{
			$helper = $this->getModel()->getContext()->getService('repositoryHelper', 'RepositoryHelper');
	...
	protected function createMapper()
	{
		return $this
			->model
			->getContext()
			->getService('mapperFactory', 'IMapperFactory')
			->createMapper($this)
		;
	}
	...
	{
		throw new OrmDeprecatedException(array(__CLASS__, 'createEntity()', __CLASS__, 'hydrateEntity()'));
	}

	/** @deprecated */
	final public function isEntity(IEntity $entity)
	{
		throw new OrmDeprecatedException(array(__CLASS__, 'isEntity()', __CLASS__, 'isAttachableEntity()'));
	}

	/** @deprecated */
	final public function getRepositoryName()
	{
		throw new OrmDeprecatedException(array(__CLASS__, 'getRepositoryName()', 'get_class($repository)'));
	}
