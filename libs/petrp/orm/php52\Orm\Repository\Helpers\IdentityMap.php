 ...
 */
class IdentityMap extends OrmObject
{
	...
	public function __construct(IRepository $repository, PerformanceHelper $performanceHelper = NULL)
	{
		...
		foreach ((array) $this->repository->getEntityClassName() as $en)
		{
			if (!class_exists($en))
			{
				throw new InvalidEntityException(get_class($this->repository) . ": entity '$en' does not exists; see property Repository::\$entityClassName or method IRepository::getEntityClassName()");
			}
