...
class DataSourceCollection extends BaseDibiCollection
{
	...
	{
		throw new OrmNotSupportedException('Joins are not supported for DataSourceCollection');
	}
	...
	{
		throw new OrmDeprecatedException(array(__CLASS__, 'fetchSingle()', __CLASS__ . '->getDataSource()->fetchSingle()'));
	}

	/** @deprecated */
	final public function select($col, $as = NULL)
	{
		throw new OrmDeprecatedException(array(__CLASS__, 'select()', __CLASS__ . '->getDataSource()->select()'));
	}
}
