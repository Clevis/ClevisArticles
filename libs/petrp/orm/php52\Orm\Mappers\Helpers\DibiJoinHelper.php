 ...
 */
class DibiJoinHelper extends OrmObject
{
	/** @var DibiMapper */
	private $mapper;
	...
	private function getInfo($sourceKey)
	{
		if (!isset($this->cache[$sourceKey]))
		{
			...
			{
				throw new MapperJoinException(get_class($joinRepository) . " ($sourceKey) nepouziva DibiMapper, data nelze propojit.");
			}
			...
			if ($tmp['mapper']->getConnection() !== $this->mapper->getConnection())
			{
				...
				throw new MapperJoinException(get_class($joinRepository) . " ($sourceKey) pouziva jiny DibiConnection nez " . get_class($this->mapper->getRepository()) . ", data nelze propojit.");
			}
			...
			{
				throw new MapperJoinException(get_class($joinRepository) . " ($sourceKey) nepouziva DibiCollection, data nelze propojit.");
			}
			$collectionArray = (array) $collection; // hack
			if ($collectionArray["\0*\0where"])
			{
				throw new MapperJoinException(get_class($joinRepository) . " ($sourceKey) DibiCollection pouziva where(), data nelze propojit.");
			}
