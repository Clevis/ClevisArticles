 ...
 * @subpackage DI
 */
class ServiceContainer extends OrmObject implements IServiceContainer
{

	/** @var array of name => OrmObject */
	...
	/**
	 * <code>
	 * 	$c->addService('foo', function (IServiceContainer $c) { return new Foo; });
	...
	public function getService($name, $instanceof = NULL)
	{
		...
		if ($s->service === NULL)
		{
			...
			else if (OrmCallback::is($s->factory) OR $s->factory instanceof Closure OR is_string($s->factory) OR is_array($s->factory))
			{
				$tmp = OrmCallback::create($s->factory)->invoke($this);
				if (!is_object($tmp))
				{
					$tmp = gettype($tmp);
					throw new InvalidServiceFactoryException("Factory for service '$name' returns invalid result. OrmObject expected, $tmp given.");
				}
				$s->service = $tmp;
				unset($s->factory);
			}
