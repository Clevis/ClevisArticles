 ...
 */
class MetaData extends OrmObject
{
	...
	/** @var string */
	private $propertyClass = 'MetaDataProperty';
	 ...
	 * @param string|NULL null means default MetaDataProperty
	 */
	public function __construct($entityClass, $propertyClass = NULL)
	{
		...
		else
		{
			...
			{
				throw new OrmInvalidArgumentException(array($this, '$entityClass', 'instance of IEntity', '', "; class '$entityClass' doesn't exists"));
			}
			$r = new ReflectionClass($entityClass);
			$entityClass = $r->getName();
			if (!$r->implementsInterface('IEntity'))
			{
				throw new OrmInvalidArgumentException(array($this, '$entityClass', 'instance of IEntity', $entityClass));
			}
		}
		...
		if ($propertyClass !== NULL)
		{
			...
			if (!is_subclass_of($propertyClass, 'MetaDataProperty') AND strcasecmp($propertyClass, 'MetaDataProperty') !== 0)
			{
				throw new OrmInvalidArgumentException(array($this, '$propertyClass', 'subclass of MetaDataProperty', $propertyClass));
			}
	...
	public function getMethods($name)
	{
		if ($this->methods === NULL)
		{
			...
			if ($excludeMethods === NULL)
			{
				...
				$excludeMethods = get_class_methods('BaseEntityFragment');
	...
	private static function createEntityRules($entityClass)
	{
		...
		if (!isset($implements['IEntity'])) throw new InvalidEntityException("'$entityClass' isn`t instance of IEntity");
		$meta = call_user_func(array($entityClass, 'createMetaData'), $entityClass);
		if (!($meta instanceof MetaData)) throw new BadReturnException(array($entityClass, 'createMetaData', 'MetaData', $meta));
		return $meta;
	}
