	...
	public static function parse(& $name, array & $args)
	{
		...
		if ($mode AND $by)
		{
			...
			foreach (explode('And', $by) as $n => $key)
			{
				...
				{
					throw new OrmInvalidArgumentException("There is no value for '$key' in '$name'.");
				}
				$where[$key] = $args[$n];
				unset($args[$n]);
			}
			if (count($args))
			{
				throw new OrmInvalidArgumentException("There is extra value in '$name'.");
			}
			$name = $mode;
			$args = $where;
			return true;
