 ...
 */
class ArrayCollection extends OrmObject implements IEntityCollection
{
	...
	final public function orderBy($key, $direction = self::ASC)
	{
		...
		else
		{
			...
			if ($direction !== self::ASC AND $direction !== self::DESC)
			{
				...
				throw new OrmInvalidArgumentException(array($this, 'orderBy() $direction', 'IEntityCollection::ASC or DESC', $direction));
			}
	...
	final public function fetchPairs($key = NULL, $value = NULL)
	{
		...
		if ($value === NULL)
		{
			throw new OrmInvalidArgumentException("Value or both columns must be specified.");
		}
		else if (!$row->hasParam($value))
		{
			throw new OrmInvalidArgumentException("Unknown value column '$value'.");
		}
		else if ($key !== NULL AND !$row->hasParam($key))
		{
			throw new OrmInvalidArgumentException("Unknown key column '$key'.");
		}
			...
			foreach ($where as $key => $value)
			{
				if (strpos($key, '->') !== false)
				{
					...
					foreach (explode('->', $key) as $i => $k)
					{
						...
						{
							throw new OrmInvalidArgumentException("'$k' is not key in '{$key}'");
						}
						else
						{
							$eValue = $eValue->{$k};
		...
		foreach ($this->_sort as $tmp)
		{
			...
			else
			{
				if (!$aRow->hasParam($key) OR !$bRow->hasParam($key))
				{
					...
					{
						throw new OrmInvalidArgumentException("'$key' is not key");
					}
				}
			...
			else
			{
				...
				throw new OrmInvalidArgumentException(get_class($aRow) . "::\$$key contains non-sortable value, $tmp");
			}
