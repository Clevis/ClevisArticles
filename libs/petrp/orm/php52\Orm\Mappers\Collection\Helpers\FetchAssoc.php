...
class FetchAssoc
{
	 ...
	 * @throws OrmNotSupportedException for modifer '->'
	...
	final public static function apply(array $rows, $assoc)
	{
		...
		foreach ($assoc as $as)
		{
			...
			{
				throw new OrmInvalidArgumentException("Unknown column '$as' in associative descriptor.");
			}
		}
		...
		do {
			...
			foreach ($assoc as $i => $as)
			{
				...
				{
					throw new OrmNotSupportedException('FetchAssoc "object" node (->) is not supported');
				}
	 ...
	 * @return array
	 * @throws OrmNotSupportedException for modifer '=' and '@'
	...
	private static function oldFetchAssoc(array $rows, $assoc, IEntity $row)
	{
		...
		do {
			...
			foreach ($assoc as $i => $as)
			{
				...
				{
					throw new OrmNotSupportedException('FetchAssoc "record" node (=) is not supported');
				}
				else if ($as === '@') // "object" node
				{
					throw new OrmNotSupportedException('FetchAssoc "object" node (@) is not supported');
