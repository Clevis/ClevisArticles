 ...
 */
class Events extends OrmObject
{
	/**
	...
	private static $instructions = array(
		self::HYDRATE_BEFORE => array('IListenerHydrateBefore', 'onBeforeHydrateEvent', 'onLoad'),
		self::HYDRATE_AFTER => array('IListenerHydrateAfter', 'onAfterHydrateEvent', NULL),
		self::ATTACH => array('IListenerAttach', 'onAttachEvent', 'onAttach'),
		self::PERSIST_BEFORE => array('IListenerPersistBefore', 'onBeforePersistEvent', 'onBeforePersist'),
		self::PERSIST_BEFORE_UPDATE => array('IListenerPersistBeforeUpdate', 'onBeforePersistUpdateEvent', 'onBeforeUpdate'), // todo method name
		self::PERSIST_BEFORE_INSERT => array('IListenerPersistBeforeInsert', 'onBeforePersistInsertEvent', 'onBeforeInsert'),
		self::PERSIST => array('IListenerPersist', 'onPersistEvent', 'onPersist'), // todo?
		self::PERSIST_AFTER_UPDATE => array('IListenerPersistAfterUpdate', 'onAfterPersistUpdateEvent', 'onAfterUpdate'),
		self::PERSIST_AFTER_INSERT => array('IListenerPersistAfterInsert', 'onAfterPersistInsertEvent', 'onAfterInsert'),
		self::PERSIST_AFTER => array('IListenerPersistAfter', 'onAfterPersistEvent', 'onAfterPersist'),
		self::REMOVE_BEFORE => array('IListenerRemoveBefore', 'onBeforeRemoveEvent', 'onBeforeRemove'),
		self::REMOVE_AFTER => array('IListenerRemoveAfter', 'onAfterRemoveEvent', 'onAfterRemove'),
		self::FLUSH_BEFORE => array('IListenerFlushBefore', 'onBeforeFlushEvent', NULL),
		self::FLUSH_AFTER => array('IListenerFlushAfter', 'onAfterFlushEvent', NULL),
		self::CLEAN_BEFORE => array('IListenerCleanBefore', 'onBeforeCleanEvent', NULL),
		self::CLEAN_AFTER => array('IListenerCleanAfter', 'onAfterCleanEvent', NULL),
		self::SERIALIZE_BEFORE => array('IListenerSerializeBefore', 'onBeforeSerializeEvent', NULL),
		self::SERIALIZE_AFTER => array('IListenerSerializeAfter', 'onAfterSerializeEvent', NULL),
		self::SERIALIZE_CONVENTIONAL => array('IListenerSerializeConventional', 'onConventionalSerializeEvent', NULL),
	);
	...
	public function addListener(IListener $event)
	{
		...
		if (!$is)
		{
			throw new OrmInvalidArgumentException(ExceptionHelper::format(array($this, $event), "%c1::addListener() no event interface match for '%c2'."));
		}
		return $this;
	}
	...
	{
		$is = false;
		$callback = OrmCallback::create($callback)->getNative();
		foreach (self::$instructions as $e => $m)
		{
			if ($type & $e)
			{
		...
		if (!$is)
		{
			throw new OrmInvalidArgumentException(ExceptionHelper::format(array($this, $type), "%c1::addCallbackListener() no event constant match for '%s2'."));
		}
		return $this;
	}
	...
	{
		$is = false;
		$factory = OrmCallback::create($factory)->getNative();
		$key = count($this->lazy);
		$types = array();
		foreach (self::$instructions as $e => $m)
		{
		...
		if (!$is)
		{
			throw new OrmInvalidArgumentException(ExceptionHelper::format(array($this, $type), "%c1::addLazyListener() no event constant match for '%v2'."));
		}
	...
	public function fireEvent($type, IEntity $entity = NULL, array & $arguments = array())
	{
		...
		{
			throw new OrmInvalidArgumentException(array($this, 'fireEvent() $type', 'valid event type', $type));
		}
	...
	private function handleLazy($key)
	{
		...
		{
			$cb = OrmCallback::create($this->lazy[$key][0]);
			throw new BadReturnException(array(NULL, __CLASS__ . " lazy factory $cb()", 'IListener', $object));
		}
		...
		foreach (self::$instructions as $e => $m)
		{
			if (isset($types[$e]))
			{
				...
				{
					throw new OrmInvalidArgumentException(ExceptionHelper::format(array($this, OrmCallback::create($this->lazy[$key][0]), $m[0], $object), "%c1 lazy factory %s2() must return %s3; '%v4' given."));
				}
			}
			else if ($object instanceof $m[0])
			{
				throw new OrmInvalidArgumentException(ExceptionHelper::format(array($this, OrmCallback::create($this->lazy[$key][0]), $m[0], $object), "%c1 lazy factory %s2() returns not expected %s3; '%v4'."));
			}
		}
