 ...
 */
class MetaDataProperty extends OrmObject
{
	...
	protected function setAccess($access, MetaData $meta)
	{
		...
		if (!in_array($access, array(MetaData::READ, MetaData::READWRITE), true)) throw new MetaDataException(__CLASS__ . ' access is MetaData::READ or MetaData::READWRITE allowed');
	...
	private function setToMany($relationship, $repositoryName, $param, $mapped = NULL)
	{
		...
		$loader = $relationship === MetaData::ManyToMany ? 'RelationshipMetaDataManyToMany' : 'RelationshipMetaDataOneToMany';
	...
	public function setEnum(array $values, $original = NULL)
	{
		if ($original === NULL)
		{
			$original = implode(', ', array_map(create_function('$v', '
	...
	public function setInjection($factory = NULL)
	{
		...
		if (count($this->data['types']) != 1)
		{
			throw new MetaDataException("Injection expecte type as one class implements IInjection, '{$class}' given in {$this->class}::\${$this->name}");
		}
		if (!class_exists($class))
		{
			throw new MetaDataException("Injection expecte type as class implements IInjection, '{$class}' given in {$this->class}::\${$this->name}");
		}
		$reflection = new ReflectionClass($class);
		$class = $reflection->getName();

		if (!$reflection->implementsInterface('IEntityInjection'))
		{
			throw new MetaDataException("$class does not implements IEntityInjection in {$this->class}::\${$this->name}");
		}
		...
		{
			$factory = OrmCallback::create($factory, 'create');
		}
		else if (OrmCallback::is($factory) OR $factory instanceof Closure OR (is_string($factory) AND (strpos($factory, '::') OR strncmp($factory, "\0lambda_", 8) === 0)))
		{
			$factory = OrmCallback::create($factory);
		}
		else if (!$factory AND $reflection->implementsInterface('IEntityInjectionStaticLoader'))
		{
			$factory = OrmCallback::create($class, 'create');
		}
		else
		{
			if (!$factory)
			{
				throw new MetaDataException("There is not factory callback for injection in {$this->class}::\${$this->name}, specify one or use IEntityInjectionStaticLoader");
			}
			$tmp = is_object($factory) ? get_class($factory) : (is_string($factory) ? $factory : gettype($factory));
			throw new MetaDataException("Injection expected valid callback, '$tmp' given in {$this->class}::\${$this->name}, specify one or use IEntityInjectionStaticLoader");
		}
	...
	{
		throw new OrmDeprecatedException(array($this, 'check()'));
	}
